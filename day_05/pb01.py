...
def solve(points):
    ...
    grid2 = [['.' for _y in range(max_y)] for _x in range(max_x)]
    # print(len(grid), len(grid[0]))
    # print(grid)
    for p_idx, p in enumerate(points):        # res = solve(test)
    ...
    for x in range(max_x):
        for y in range(max_y):
            ...
            all_distances = sum([d[1] for d in distances])
    ...
    for line in grid2:
        print(''.join(line))
    print('Region size: %s' % (region_size, ))
...
def solve2(points):
    ...
    for x in range(-50, max_x + 50):
        for y in range(-50, max_y + 50):
            distances = 0
            for p in points:
                d = md((x, y), p)
                distances += d
            if distances < 10000:
                print(distances)
                region_size += 1
    print('Region size 2 : %s' % (region_size, ))
